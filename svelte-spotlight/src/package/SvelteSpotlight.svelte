...
import { scrollIntoViewIfNeeded, isCombo } from './index';
...
export let combo = { key: 'k', metaKey: true };
/** Max search input length */
export let maxLength = 50;
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
export let overlayTransition = fade;
export let overlayTransitionConfig = { duration: 200 };
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
//@ts-ignore
export let modalTransitionIn = scale;
export let modalTransitionInConfig = {
    start: 0.95
};
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
//@ts-ignore
export let modalTransitionOut = scale;
export let modalTransitionOutConfig = {
    start: 0.95
};
...
export let headerCenterComponent = undefined;
...
const onKeyDown = (e) => {
    ...
    if (!noResults && isOpen) {
        ...
        const current = options.findIndex((element) => element.id === preSelectedResult?.[resultIdKey]);
        const selectResult = (position) => {
            const element = options[position];
            if (groupIdKey) {
                const group = results.find((group) => group[groupIdKey] === element.parentElement.id);
                preSelectedResult = group[groupResultsKey].find((result) => result[resultIdKey] === element.id);
            }
            else {
